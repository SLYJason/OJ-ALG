    ...
    public List<List<Integer>> verticalTraversal(TreeNode root) {
        ...
        while(!queue.isEmpty()) {
            ...
            for(int i=queue.size(); i>0; i--) {
                ...
                if(node.left != null) buffer.add(new AbstractMap.SimpleEntry<TreeNode, Integer>(node.left, col-1));
                if(node.right != null) buffer.add(new AbstractMap.SimpleEntry<TreeNode, Integer>(node.right, col+1));
            }
